...
resource "aws_instance" "main" {
  count         = length(data.tfe_outputs.networking.data.tfe_outputs.networking.nonsensitive_values.publics_subnets)
  ami           = nonsensitive(data.aws_ssm_parameter.amzn2_linux.value)
  instance_type = var.instance_type
  subnet_id     = data.tfe_outputs.networking.data.tfe_outputs.networking.nonsensitive_values.publics_subnets[count.index]
...
resource "aws_lb" "main" {
  ...
  subnets            = data.tfe_outputs.networking.data.tfe_outputs.networking.nonsensitive_values.publics_subnets
